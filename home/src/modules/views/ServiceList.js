...
import {loadAllServiceAction,loadBestServiceAction} from '../../redux/actions'
...
class ProductHowItWorks extends React.Component {
  ...
  componentWillMount() {
    const {loadAllServiceAction,loadBestServiceAction}=this.props;
    loadAllServiceAction();
    loadBestServiceAction();
  }
    renderGridService(){
      const {all_services,classes}=this.props;
      return(
        <Grid container spacing={8} className={classes.gridList}>
          {
            all_services.filter(item=>item.status==='active').map((item,index)=>{
              return(
...
const mapStateToProps=state=>{
  return{
    all_services: state.services.all,
